...
class UsersController extends Controller
{
    ...
    {
        $posts = auth()->user()->posts()->with(['media', 'category', 'user']) // طريقة2 بدل المودل عن طريق الاوث
    ...
    public function store_post(Request $request)
    {
        ...
        $data['description'] = Purify::clean($request->description); // تنضيف الاكواد للحماية
        ...
        if ($request->images && count($request->images) > 0) {
            ...
            foreach ($request->images as $file) {
                ...
                Image::make($file->getRealPath())->resize(800, null, function ($constraint) { // get from cache
                    $constraint->aspectRatio();
                })->save($path, 100);
        ...
        if ($request->status == 1) {
            Cache::forget('recent_posts'); // to make new cache
    ...
    {
        $post = Post::whereSlug($post_id)->orWhere('id', $post_id)->whereUserId(auth()->id())->first(); // id or slug  ممكن يحتوي
    ...
    public function show_comments(Request $request)
    {
        ...
        if (isset($request->post) && $request->post != '') {
            ...
            $posts_id = auth()->user()->posts->pluck('id')->toArray(); // ids for posts  that i wrote
            $comments = $comments->whereIn('post_id', $posts_id); // post_id in comments
        }
    ...
    public function edit_comment($comment_id)
    {
        $comment = Comment::whereId($comment_id)->whereHas('post', function ($query) {
            $query->where('posts.user_id', auth()->id()); // للتأكيد
        })->first();
    ...
    public function update_comment(Request $request, $comment_id)
    {
        ...
        if ($comment) {
            ...
            if ($request->status == 1) {
                Cache::forget('recent_comments'); // للحذف من الكاش
            }
