           ...
           @foreach($recent_comments as $recent_comment)
            <li>
                <div class="post-wrapper">
                    <div class="thumb">
                      <img src="{{ get_gravatar($recent_comment->email, 47) }}" alt="{{ $recent_comment->name }}">   {{--  47 image size --}}
                    </div>
                    <div class="content">
                        <p>{{ $recent_comment->name }} says:</p>
                        <a href="#">{!! \Illuminate\Support\Str::limit($recent_comment->comment, 25, '...') !!}</a>  {{-- 25 char - بعدها نقط--}}
                    </div>
                </div>
            </li>
            @endforeach
